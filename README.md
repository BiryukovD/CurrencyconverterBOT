# CurrencyconverterBOT

<p align="center">
<a href="https://t.me/Currency_Converter_01_bot" alt="Run Telegram Bot shield"><img src="https://img.shields.io/badge/RUN-Telegram%20Bot-blue?logo=data:image/svg+xml;base64," width="230"/></a>
</p>

## Описание
Telegram Bot конвертер валют. Быстрый и удобный перевод валют в соответствии с актуальными курсами.
Данные о текущем курсе валют берутся с 
<a href="https://currencyapi.com/">currencyapi.com</a> по API.

Бот поддерживает пять валют для перевода: 
- RUB - рубль
- EUR - евро
- USD - американский доллар
- JPY - японская иена
- CAD - канадский доллар

Чтобы начать работу с ботом введите команду в следующем формате:<p>`<тикер начальной валюты> <тикер валюты в которую нужно перевести> <количество переводимой валюты>`</p>


<p align="center">
<img src="images/Currency.gif" height="600" />
</p>



